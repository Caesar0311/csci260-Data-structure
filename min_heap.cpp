...
min_heap::min_heap() :m_M(DEFAULT_MINHEAP_MAX_CAPACITY), m_N(0)
{
	m_data = new arc[m_M];
}

min_heap::min_heap(int max_capacity) : m_M(max_capacity + 1), m_N(0)
{
	m_data = new arc[m_M];
}
...
void min_heap::print()
{
	...
	for (int i = 0; i <= height; i++) {
		for (int j = 0; j < pow(2, i); j++) {
			...
			std::cout << std::setw(4);
			std::cout << "("<<m_data[(int)pow(2, i) + j]->from << "," << m_data[(int)pow(2, i) + j]->to<< "," << m_data[(int)pow(2, i) + j]->distance<<")";
			std::cout << std::setw(tab / 2) << " ";
		}
		std::cout << std::endl;
	}
